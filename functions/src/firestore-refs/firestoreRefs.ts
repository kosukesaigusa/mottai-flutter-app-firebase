...
export const attendingRoomsRef = (
    { userId }: { userId: string }
): CollectionReference<AttendingRoom> => v1MessageRef
...
/** room コレクションの参照 */
export const roomsRef: CollectionReference<Room> = v1MessageRef
    .collection(`rooms`)
...
): DocumentReference<Room> => roomsRef.doc(roomId)


/** messages コレクションの参照 */
export const messagesRef = (
    { roomId }: { roomId: string }
): CollectionReference<Message> => roomRef({ roomId: roomId })
    .collection(`messages`)
...
): DocumentReference<Message> => messagesRef({ roomId: roomId }).doc(messageId)
...
export const publicUserRef = (
    { publicUserId }: { publicUserId: string }
): DocumentReference<PublicUser> => publicUsersRef.doc(publicUserId)
